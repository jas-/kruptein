...
class Kruptein {
  ...
  constructor(options) {
    ...
    this._use_safe_timing = options.use_safe_timing ?? true;
  ...
  get(secret, ciphertext, opts, cb) {
    ...
    if (hmac_bytes.byteLength !== orig_hmac_bytes.byteLength) {
      ...
      if (this._use_safe_timing) {
        try {
          valid = this.crypto.timingSafeEqual(hmac_bytes, orig_hmac_bytes);
        } catch(err) {
          // This shoudn't be reached, but if it does provide some info on why
          console.error(err);
          console.log(hmac_bytes);
          console.log(orig_hmac_bytes);
