...
class Kruptein {
  ...
  set(secret, plaintext, aad, cb) {
    ...
    let iv, ct, hmac, obj, key, file_name, is_file = false;
    ...
    // Is plaintext a file?
    this.fs.access(plaintext, this.fs.constants.R_OK, (err) => {
      // An error is returned assume 
      if (err)
        return;
console.log(1)
      is_file = true;
      file_name = plaintext+".kruptein";

      // Get the contents of the file
      this.fs.open(plaintext, "r", (err, res) => {
        if (err)
...
console.log(1)
      ...
      this.fs.open(file_name, "w", (err, fd) => {
        if (err)
          return cb("Unable to create file handle!");

        // Write ciphertext object to handle
        this.fs.write(fd, JSON.stringify(obj), (err, res) => {
          if (err)
            return cb("Unable to write ciphertext to file handle!");
        });
      });
    } else {
  ...
  get(secret, ciphertext, opts, cb) {
    ...
    this._decrypt(key.key, ct.ct, this.algorithm, this.encodeas,
                  Buffer.from(ct.iv, this.encodeas), ct.at, ct.aad, (err, res) => {
      if (err)
  ...
  _derive_key(secret, salt, cb) {
    ...
    //   References: RFC 4806, 5084, SP 800-57P1, 800-108 & 800-132
