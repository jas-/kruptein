...
class Kruptein {
  ...
  constructor(secret, options) {
    ...
    // Singleton: Key cache for derived keys
    this._keyCache = null;
  }

  set(plaintext, aad, cb) {
    cb = cb || aad;
    ...
    if (!this._complexity(this._secret))
      return cb("The supplied secret failed to meet complexity requirements!");

    // Check if key is already cached
    if (this._keyCache) {
      key = this._keyCache;
      this._continueEncryption();
    } else {
      this._derive_key(this._secret, (err, secret) => {
        if (err)
          return cb("Unable to derive key!");

        key = secret;
        this._keyCache = secret; // Cache the derived key
        this._continueEncryption();
      });
    }
    ...
    function _continueEncryption() {
      iv = self._iv(self._iv_size);
      ...
      if (self._aead_mode && typeof aad === "function") {
        self._digest(self._iv(128), plaintext, self._hashing, self._encodeas, (err, res) => {
          if (err)
      ...
      self._encrypt(key.key, plaintext, self._algorithm, self._encodeas, iv, aad, (err, ciphertext) => {
        if (err)
      ...
      self._digest(key.key, material, self._hashing, self._encodeas, (err, digest) => {
        if (err)
      ...
      return (self._use_asn1) ?
        cb(null, self.schema.encode(obj).toString(self._encodeas)) :
  ...
  get(ciphertext, opts, cb) {
    ...
    // Check if we have the key cached with matching salt
    if (this._keyCache && Buffer.compare(this._keyCache.salt, Buffer.from(ct.salt, this._encodeas)) === 0) {
      key = this._keyCache;
      this._continueDecryption();
    } else {
      this._derive_key(this._secret, ct.salt, (err, secret) => {
        if (err)
          return cb("Unable to derive key!");
    ...
    function _continueDecryption() {
      material = Buffer.from(ct.ct + ct.salt + ct.iv);
      self._digest(key.key, material, self._hashing, self._encodeas, (err, res) => {
        if (err)
      ...
      if (hmac_bytes.byteLength !== orig_hmac_bytes.byteLength) {
        ...
        if (typeof self.crypto.timingSafeEqual === "function") {
          try {
            valid = self.crypto.timingSafeEqual(hmac_bytes, orig_hmac_bytes);
          } catch(err) {
      ...
      if (ct.at)
        ct.at = Buffer.from(ct.at, self._encodeas);

      self._decrypt(key.key, ct.ct, self._algorithm, self._encodeas, Buffer.from(ct.iv, self._encodeas), ct.at, ct.aad, (err, res) => {
        if (err)
  ...
  // Public method to clear cached key
  clearCache() {
    this._keyCache = null;
  }
}
...
module.exports = function(secret, options) {
  if (!instance) {
    instance = new Kruptein(secret, options || {});
  }
  return instance;
};

// Export for testing purposes
module.exports.resetInstance = function() {
  if (instance) {
    instance.clearCache();
    instance = null;
  }
