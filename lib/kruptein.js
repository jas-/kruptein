...
class Kruptein {
  ...
  async _derive_key(secret, salt, cb) {
    ...
    try {
      if (this._use_scrypt && typeof this.crypto.scryptSync === "function") {
        key = await this.crypto.scryptSync(secret, salt, this._key_size, opts);
      } else if (this._use_argon2 && typeof this.crypto.argon2Sync === "function") {
        parameters.message = secret;
        parameters.nonce = salt;
        key = await this.crypto.argon2Sync('argon2id', parameters);
      } else {
        key = await this.crypto.pbkdf2Sync(secret, salt, 20000, this._key_size, this._hashing);
      }
    } catch (err) {
      return cb("Unable to derive key!");
    }
