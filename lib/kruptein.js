...
class Kruptein {
  ...
  get(secret, ciphertext, opts, cb) {
    ...
    if (hmac.length !== ct.hmac.toString().length) {
      return cb("Encrypted session was tampered with!");
    } else {
      // This should never be reached, but AI says it is a probablity
